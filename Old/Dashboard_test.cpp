...
void setup() {
  ...
  g_config.rx_queue_len = 20; // FIX: 20-message queue to handle 20Hz updates
...
void loop() {
  ...
  // --- 2. SERIAL INPUT (Manual Override on Dash) ---
  if (Serial.available()) {
  ...
  // FIX: Reduced to 5ms to allow maximum framerate for the 20Hz Simulator
  delay(5); 
}