 ...
 * Symmetrical Node-to-Node Communication (Event-Driven)
 */
...
// =========================================================================
// 1. NODE IDENTIFICATION TOGGLE
// Set to 1 for your FIRST display, set to 2 for your SECOND display!
// =========================================================================
#define NODE_ID 1
...
// Both PCBs are identical, so they use the same CAN pins
...
// =========================================================================
// GAUGE & SCREEN DRAWING (Unchanged)
// =========================================================================
...
void broadcastData(uint32_t id, uint8_t* payload, uint8_t length) {
  ...
  // FIXED: Changed timeout from 0 to 5ms so messages don't drop if the queue gets full!
...
void setup() {
  ...
  // FIXED: Increased the TX Queue from 5 to 10 so all 6 frames fit without overflowing
...
void loop() {
  ...
  // Determine Base IDs based on which Node this is
  ...
  while (twai_receive(&rx_msg, 0) == ESP_OK) {
    // Only process messages from the OTHER node
    if (rx_msg.identifier >= listen_rx_base && rx_msg.identifier <= listen_rx_base + 5) {
      uint32_t offset = rx_msg.identifier - listen_rx_base;
      
      switch (offset) {
        case 0:
          memcpy(&oilTemp, &rx_msg.data[0], 4); memcpy(&oilPress, &rx_msg.data[4], 4); break;
        case 1:
          memcpy(&engineWaterTemp, &rx_msg.data[0], 4); memcpy(&icWaterTemp, &rx_msg.data[4], 4); break;
        case 2:
          memcpy(&lambdaVal, &rx_msg.data[0], 4); memcpy(&intakeTemp, &rx_msg.data[4], 4); break;
        case 3:
          memcpy(&egt, &rx_msg.data[0], 4); memcpy(&batteryVolts, &rx_msg.data[4], 4); break;
        case 4:
          memcpy(&boostPressure, &rx_msg.data[0], 4); memcpy(&hybridTemp, &rx_msg.data[4], 4); break;
        case 5:
          memcpy(&hybridVolts, &rx_msg.data[0], 4);
          currentGear = rx_msg.data[4]; stateOfCharge = rx_msg.data[5]; break;
      }
    }
  }
  ...
  if (Serial.available()) {
    ...
    if (input.startsWith("C")) {
      ...
      // If it's not a screen change, it's a data update!
      ...
      // Flag that we need to share this update with the network
      localChangeMade = true;
    }
  }

  // --- 3. EVENT-DRIVEN CAN BROADCAST ---
  // We ONLY broadcast if we just updated a value via the Serial Monitor.
  if (localChangeMade) {
